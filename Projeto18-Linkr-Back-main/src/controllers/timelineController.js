...
export async function getPosts(req, res) {
  try {
    const { userInfo } = res.locals; // Checando se há informações do usuário
    const userId = userInfo ? userInfo.user_id : null; // Definindo o userId se disponível

    // Se userInfo estiver disponível, recuperamos posts com metadata
    if (userInfo) {
      const { rows: posts } = await timelineRepository.getAllPosts();
      const metadataPosts = await Promise.all(
        posts.map(async ({ id, likes, username, photo, url, post, userId }) => {
      ...
      const result = { metadataPosts, userId };
      console.log(result);
      return res.status(200).send(result);
    }

    // Caso contrário, apenas recuperamos posts simples sem metadata
    const posts = await timelineRepository.getAllPosts();
    return res.send(posts.rows);
  } catch (err) {
    return res.status(STATUS_CODE.SERVER_ERROR).send(err.message);
